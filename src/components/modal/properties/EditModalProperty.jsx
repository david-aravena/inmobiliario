import styles from './editModalProperties.module.css'
import {deleteFile} from 'app/serverless/utils/storage/'
import {deleteFileUrl, updateImages} from 'app/serverless/utils/db/'
import {saveNewImages} from './utils/editModalProperty'
...
export default function EditModalProperty({itemSelected, setItemSelected}){

  const deleteImageHandler = async (urlImage, id) => {
    const isConfirmed = window.confirm("¿Estás seguro de que deseas eliminar este elemento?");
    if (isConfirmed) {
      await deleteFile(urlImage)
      await deleteFileUrl(id, urlImage)
      const newItemSelected = itemSelected.images.filter(image => image !== urlImage);
      setItemSelected({...itemSelected, images: newItemSelected})
  ...
  const getNewArrayImages = async () => {
    const newImages = itemSelected.images.filter((url) => url instanceof File);
    const oldImages = itemSelected.images.filter((url) => !(url instanceof File));
    const savedNewImages = await saveNewImages(newImages)
    updateImages([...savedNewImages, ...oldImages], itemSelected.id)
        ...
        <div className={styles.infoContainer}>
          <div className={styles.imagesContainer}>
            <div className={styles.imageSelectedContainer}>
              {itemSelected.images.length && (
                <img 
                  src={itemSelected.images[itemSelected.imageSelected] instanceof File ? (URL.createObjectURL(itemSelected.images[itemSelected.imageSelected]) || itemSelected.images[0]) : (itemSelected.images[itemSelected.imageSelected] || itemSelected.images[0])  } 
                  alt="error" 
                  height={"100%"} 
              ...
              <div className={styles.buttonDeleteContainer}>
                <button style={{padding:"1rem"}} onClick={() => deleteImageHandler(itemSelected.images[itemSelected.imageSelected], itemSelected.id)}>Borrar</button>
              </div>
            </div>
            <div>
              <input
                ...
                style={{padding:"1rem"}}
              />
              <button style={{padding:"1rem"}} onClick={() => getNewArrayImages()}>Actualizar imagenes</button>
            </div>
            <div style={{
              width: "100%", 
              padding: "1rem 0", 
          ...
          <div className={styles.detailsContainer}>
            <div style={{padding:"0 0 1rem 0"}}>
              <button className={styles.buttonContact}>Contactar al vendedor</button>
            </div>
